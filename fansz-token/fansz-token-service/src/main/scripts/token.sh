#!/bin/sh
MAIN_CLASS="com.fansz.token.TokenStarter"
...
#star auth server
nohup "$JAVA_HOME"/bin/java  $JAVA_OPTS  -Dapplication.root="$BASE_DIR" -Dlog4j.configuration="file:$BASE_DIR/conf/log4j.xml" -classpath "$BASE_DIR/lib/*" "$MAIN_CLASS"  >> "$TOKEN_OUT" 2>&1 &
