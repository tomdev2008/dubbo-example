#!/bin/sh
MAIN_CLASS="com.fansz.fandom.FandomStarter"
...
FANDOM_OUT="$LOG_PATH/fandom.out"
...
#star fandom server
nohup "$JAVA_HOME"/bin/java  $JAVA_OPTS -Dio.netty.allocator.type=pooled -Dapplication.root="$BASE_DIR" -Dlog4j.configuration="file:$BASE_DIR/conf/log4j.xml" -classpath "$BASE_DIR/lib/*" "$MAIN_CLASS"  >> "$FANDOM_OUT" 2>&1 &
